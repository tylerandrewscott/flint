...
data_temp = left_join(data_temp, debt_df %>% mutate(special_purpose = abs(general_purpose-1)) %>%
                        ...
                        complete(COUNTY_FIPS, Year, fill = list(sd_count = 0)))
...
data_temp = data_temp %>%
  mutate(Debt_Ratio_LGonly = (LG_DEBT/Total_Assessed_Value) * 100,
         ...
         Income_Per_Capita_1k = Income_Per_Capita/1000,
         TotOE1m = TotOE/1000000,TotOE_LG1m = TotOE_LG/1000000,TotOE_SE1m = TotOE_SD/1000000)
...
data_temp = data_temp %>% mutate(urbanized = ifelse(metro,'Metro',ifelse(rural,'Rural','Suburban')))
...
form_ratio_allsd = Debt_Ratio_Total ~ Pop10k_mc + pop_growth_percent_mc + 
  ...
  urbanized +
  sd_count - 1 +
  f(Linear_Year,model='rw2',group=ID)+ 
  f(ID, model="bym", graph="ga_county.adj")


mod_gof = inla(form_ratio_allsd,family = 'gaussian', data = data_temp,
          control.compute = list(waic=TRUE,dic=TRUE,cpo=TRUE),
          control.inla=list(int.strategy = "grid", diff.logdens = 4,strategy = "laplace", 
                            npoints = 21))

# form_list = lapply(grep('form_ratio',ls(),value=T),get)
# 
# mod_list = lapply(form_list,function(x) inla(x,family = 'gaussian', data = data_temp,
#                                              control.compute = list(waic=TRUE,dic=TRUE,mlik=TRUE,cpo=TRUE)))
# 
# model_coefs = do.call(rbind,lapply(1:length(mod_list),function(x) as.data.frame(mod_list[[x]]$summary.fixed) %>% 
#                                      mutate(Model = x,Coef = mod_list[[x]]$names.fixed))) %>% filter(!grepl('Intercept',Coef))

save.image('Scratch/laplace_estimation.RData')
